...
from syntax import OF, of_number, of_string
...
from syntax import of_range_address2
...
#Syntax test
    def test_of_formula(self):
        ...
        self.assertEqual(OF("SUM([.A1:.A2])", forcerecalc=False), expected)
        expected = "of:== SUM([.A1:.A2])"
        self.assertEqual(OF("SUM([.A1:.A2])", forcerecalc=True), expected)
        
    def test_of_intro(self):
        expected = "of:= "
        self.assertEqual(OF("", forcerecalc=False), expected)
        expected = "of:== "
        self.assertEqual(OF("", forcerecalc=True), expected)
